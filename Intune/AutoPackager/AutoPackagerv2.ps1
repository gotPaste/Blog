...
function Update-RequirementRuleForApp {
    ...
    if ($usedModule) {
        Write-Log "Requirement rule updated via module for secondary app." 'INFO'
        return $true
    }
    ...
    try {
        ...
        if ($appNameForDisplay -and $appNameForDisplay.Trim()) { $displayNameForRule = "Presence - $appNameForDisplay" }
...
# Update primary required assignments' deadline based on Rings max delay from recipe
$deadlineResult = $null
try {
